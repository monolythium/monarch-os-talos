...
Releases ship from [**github.com/monolythium/monarch-os-talos/releases**](https://github.com/monolythium/monarch-os-talos/releases). `v0.1.9` is the current signed release: it bakes the signed `protocore v0.3.0-testnet` node binary, boots **enrollment-free**, resolves the live genesis from the chain-registry, and syncs as a full node. Every artifact carries a SHA-256 checksum, a cosign keyless signature (`.sig` + `.pem`), an SPDX SBOM, and a GitHub artifact attestation.
