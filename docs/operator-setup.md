...
Releases ship from [**github.com/monolythium/monarch-os-talos/releases**](https://github.com/monolythium/monarch-os-talos/releases). `v0.1.8` is the current signed release: it bakes the signed `protocore v0.2.4-testnet` node binary, boots **enrollment-free**, resolves the live genesis from the chain-registry, and syncs as a full node. Every artifact carries a SHA-256 checksum, a cosign keyless signature (`.sig` + `.pem`), an SPDX SBOM, and a GitHub artifact attestation.
...
hcloud-upload-image upload --image-path monarch-os-talos-v1.13.0-amd64.raw.xz \
  --compression xz --architecture x86 --description "Monarch OS v0.1.8"
```
