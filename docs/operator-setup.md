...
The path is: **verify and boot the signed Monarch OS image → install Monarch Desktop → follow the in-app welcome checklist → join or form a cluster.**
...
Releases ship from [**github.com/monolythium/monarch-os-talos/releases**](https://github.com/monolythium/monarch-os-talos/releases). `v0.1.0` is the first production (non-preview) signed release: it bakes a signed `protocore` node binary, boots, resolves the live genesis from the chain-registry, and runs the node. Every artifact carries a SHA-256 checksum, a cosign keyless signature (`.sig` + `.pem`), an SPDX SBOM, and a GitHub artifact attestation.
...
3. Boot from USB. Talos installs to the internal disk and reboots into the immutable OS. Residential connectivity (NAT, dynamic IP) is supported by design.
...
hcloud-upload-image upload --image-path monarch-os-talos-v1.13.0-amd64.raw.xz \
  --compression xz --architecture x86 --description "Monarch OS v0.1.0"
```
...
## 3. First boot — what the node does on its own

You don't configure a chain. On first boot the `monarch-protocore` extension:

1. **Creates a sealed per-node operator identity** — the node's consensus and seal keys are generated on-device. No key material ships in the image.
2. **Resolves the genesis dynamically from the public chain-registry.** The image bakes *who to trust* (the registry location), not *what to run* (no hard-coded genesis): the node fetches the published genesis for `testnet-69420` over in-process TLS, verifies its hash against the registry pin, writes it locally, and loads the registry's published peer multiaddrs. A chain reset is therefore picked up automatically on any fresh boot. If resolution fails (registry unreachable, hash mismatch), the node falls back **loudly** to the baked genesis — which may be stale — unless `PROTOCORE_GENESIS_FALLBACK=fail` is set, in which case it refuses to boot.
3. **Starts `protocore` and syncs chain-69420.** The current testnet runs 2 clusters × 10 operators (7-of-10 threshold each) plus 2 relays; genesis `0x0d2903ae…` (full hash and public RPC/peer endpoints in [`chain-registry/chains/testnet-69420.toml`](https://github.com/monolythium/chain-registry/blob/master/chains/testnet-69420.toml)).
...
Check that `genesisHash` and `chainId` match the chain-registry entry and that the block number advances.

## 4. Install Monarch Desktop

Monarch Desktop is the operator console — everything from here on happens in it. Download a signed build for macOS, Windows, or Linux from [**github.com/monolythium/monarch-desktop/releases**](https://github.com/monolythium/monarch-desktop/releases) (v0.0.13 or later). It talks to your node over two channels: the Talos API (mTLS, TCP `50000`, control plane) and `protocore` JSON-RPC (TCP `8545`, data plane) — see [`monarch-desktop-connectivity.md`](./monarch-desktop-connectivity.md).

On a fresh install, Desktop opens the **welcome screen**: a ten-step checklist whose state is **detected from your node and the chain, not remembered** — you can close the app at any point and resume later; done steps stay done because they are re-probed, not ticked.

## 5. The ten-step welcome checklist

The checklist is the authoritative flow; this section explains what each step does so you know what you're agreeing to.

1. **Flash the Monarch OS image.** Sections 1–3 of this guide. The checklist links the signed releases page; the step shows done once a provisioned node is reachable.

2. **Pair Monarch with your node.** Connect the Talos control channel (your `talosconfig`) and confirm the RPC handshake on the Install page. Desktop pins the Talos CA fingerprint so later sessions fail closed on identity change.
