...
prove_enrollment_runtime() {
  ...
  jq -e \
    ...
    '
      ...
      and ($m.secret_files.operator_identity_key | type == "string")
  ...
  if [[ "$REQUIRE_TPM_BINDING_RUNTIME_PROOF" == "true" ]]; then
    local quote_path event_log_path sealed_path dkg_path quote_hash_json event_log_hash_json sealed_hash_json dkg_hash_json
    ...
    [[ "$dkg_path" == "$PROTOCORE_DKG_TRANSCRIPT_FILE" ]] || {
      enrollment_runtime_proof="failed"
      echo "DKG transcript path mismatch: manifest=$dkg_path env=$PROTOCORE_DKG_TRANSCRIPT_FILE" >&2
      exit 1
    }
    ...
    printf '%s\n' "$dkg_hash_json" >>"$ENROLLMENT_FILE_HASHES.items"
