#!/usr/bin/env bash
# Release-drift guard: assert that the staged genesis, the embedded protocore
# binary, the protocore release tag, and the chain id all agree with the
# chain-registry entry for this network. chain-registry is the single source of
# truth for the live chain; a Monarch OS image must never ship something that
# disagrees with it.
#
...
#   GENESIS_TOML       staged genesis to verify             (required)
#   PROTOCORE_BINARY   extracted ./protocore to verify      (required unless GENESIS_ONLY=1)
#   PROTOCORE_TAG      protocore release tag baked in        (required unless GENESIS_ONLY=1)
#   CHAIN_ID           chain id baked into the image        (required unless GENESIS_ONLY=1)
...
#                      registry entry + canonical genesis are read from disk
#                      (OFFLINE mode) instead of fetched over https.
...
# --- (a) resolve the registry entry + canonical genesis -------------------
...
# --- (b) require the pins we depend on -------------------------------------
