...
PROFILE="$BUILD_DIR/profile-metal.yaml"
...
RAW_SRC="$OUT_DIR/metal-$ARCH.raw"
RAW_DST="$OUT_DIR/monarch-os-talos-$TALOS_VERSION-$ARCH.raw"

if [[ ! -f "$RAW_SRC" ]]; then
  echo "expected imager output not found: $RAW_SRC" >&2
  exit 1
fi
