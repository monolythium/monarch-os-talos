...
check_jq "Desktop DKG re-share attestation evidence is missing or malformed" \
  'def clean_hex: ascii_downcase | sub("^0x"; "");
   def even_hex: type == "string" and (clean_hex | test("^[0-9a-f]+$")) and ((clean_hex | length) % 2 == 0);
   def uint56_string:
     type == "string"
     and test("^[1-9][0-9]*$")
     and ((length < 17) or (length == 17 and . <= "72057594037927935"));
   .dkg_reshare_attestation as $d
